...
const getUserStatusInfo = (user: any) => {
  ...
  switch (sub.status) {
    ...
    case SubscriptionStatus.trial:
      return { label: "Trial", color: "bg-cyan-500/20 text-cyan-400" };
    case SubscriptionStatus.grace:
      return { label: "Grace", color: "bg-orange-500/20 text-orange-400" };
    default:
      return { label: sub.status, color: "bg-zinc-800 text-zinc-500" };
  }
};
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex flex-col overflow-hidden">
                  <span
                    ...
                    title={user.telegramId?.toString()}
                  >
                    {`TG: ${user.telegramId || "NO ID"} - ${user.email || "NO USERNAME"}`}
                  </span>
                  <div className="flex items-center gap-2 text-sm text-zinc-400 mt-1">
                    <span>
                      {user.zodiacSign ? ZODIAC_SYMBOLS[user.zodiacSign] : "—"}
                    </span>
                    <span className="capitalize">
                      {user.zodiacSign || "Без знака"}
                    </span>
                  </div>
                </div>
                <span
                  className={`inline-flex px-2 py-1 rounded text-xs font-medium whitespace-nowrap ${statusInfo.color}`}
                >
                  {statusInfo.label}
                </span>
              </div>
              ...
              <div className="text-sm text-zinc-400 border-t border-zinc-800 pt-3 space-y-2">
                <div className="flex justify-between">
                  <span>Тариф:</span>
                  <span
                    className={`capitalize ${sub ? "text-cyan-400" : "text-zinc-600"}`}
                  >
                    {sub?.plan.name || "Нет"}
                  </span>
                </div>
              ...
              {users.map((user) => {
                ...
                return (
                  ...
                  >
                      ...
                      {user.zodiacSign ? (
                        ...
                        >
                          <span className="text-lg leading-none">
                            {ZODIAC_SYMBOLS[user.zodiacSign]}
                          </span>
                    ...
                    <td className="p-4">
                      <span
                        className={`capitalize text-sm ${sub?.plan.name ? "text-cyan-400" : "text-zinc-600"}`}
                      >
                        {sub?.plan.name || "—"}
                      </span>
                    </td>
                    <td className="p-4">
                      <span
                        className={`inline-flex px-2 py-1 rounded text-xs font-medium ${statusInfo.color}`}
                      >
                        {statusInfo.label}
                      </span>
                    </td>
